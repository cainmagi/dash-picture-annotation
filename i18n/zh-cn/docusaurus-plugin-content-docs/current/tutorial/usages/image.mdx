...
需要特别小心的是，这类数据URL的长度是有限的。不同的浏览器有不同的长度限制。参见[数据URL文档<IconExternalLink/>][link-dataurllim]。在某些情况下，哪怕数据长度没有超出限制，也可能会存在一些预期外的表现。因此，建议用户仅在**图片尺寸不超过`8 MB`**的情况下，使用该方案。
